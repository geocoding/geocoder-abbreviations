...
fn prepare(v: Vec<String>) -> Result<HashMap<String, Vec<Token>>, Error> {
    ...
    for lc in &v {
        ...
        for tk in &parsed {
            // println!("tk {:?}", tk); // we are pulling in the tokens here
...
mod tests {
    ...
    #[test]
    fn test_config() {
        let lcs = config(vec![String::from("dde"), String::from("edn")]).unwrap();
        assert_eq!(lcs.len(), 2);
    ...
    fn test_prepare() {
        ...
        assert_eq!(lcs.len(), 2);
        assert!(lcs.contains_key("dde"));
        assert!(lcs.contains_key("den"));
    }
